              ...
              {isAuthenticated && cart && cart.totalItems > 0 && (
                <div className="absolute top-12 right-0 mt-2 w-[320px] bg-white border-2 border-[#1A4331] shadow-[4px_4px_0px_#1A4331] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 flex flex-col pointer-events-auto cursor-default">
                  <div className="p-3 border-b-2 border-[#1A4331]/10 bg-[#F8F5F0]">
                    <p className="text-xs font-bold text-[#1A4331] opacity-70 uppercase tracking-wide">
                      Sản phẩm mới thêm
                    </p>
                  </div>
                  <div className="flex flex-col">
                    {cart.items.slice(0, 3).map((item) => (
                      <div
                        key={item.id}
                        className="flex gap-3 p-3 border-b border-[#1A4331]/5 hover:bg-[#F8F5F0] transition-colors"
                      >
                        <img
                          src={
                          ...
                          className="w-12 h-12 object-cover border border-[#1A4331]/10 bg-white"
                        />
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-bold text-[#1A4331] truncate">
                            {item.productName}
                          </p>
                  ...
                  {cart.totalItems > 3 && (
                    <div className="p-2 text-center bg-[#F8F5F0] border-t border-[#1A4331]/10">
                      <p className="text-[11px] font-bold text-[#8A9A7A]">
                        Xem thêm {cart.totalItems - 3} món trong giỏ...
                      </p>
                    </div>
                  )}
                  <div className="p-3">
                    <Link
                      to="/cart"
                      className="block w-full py-2 bg-[#1A4331] text-white text-center text-sm font-bold hover:bg-[#8A9A7A] transition-colors cursor-pointer border-2 border-transparent hover:border-[#1A4331]"
                    >
                      XEM THÊM GIỎ HÀNG
                    </Link>
                  </div>
              ...
              {isAuthenticated && (
                <>
                  ...
                  <div className="px-2 space-y-2">
                    <Link
                      to="/profile"
                      className="flex items-center gap-3 py-2 px-2 text-sm font-bold text-[#1A4331] hover:bg-[#1A4331] hover:text-[#F8F5F0] uppercase transition-colors"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      <UserCircle className="h-5 w-5" /> Thông tin cá nhân
                    </Link>
                    <Link
                      to="/profile/address"
                      className="flex items-center gap-3 py-2 px-2 text-sm font-bold text-[#1A4331] hover:bg-[#1A4331] hover:text-[#F8F5F0] uppercase transition-colors"
                    ...
                    <Link
                      ...
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      <PackageSearch className="h-5 w-5" /> Đơn hàng của tôi
                    </Link>
                    <button
                      onClick={handleLogout}
                      className="flex items-center gap-3 py-2 px-2 text-sm font-bold text-red-600 w-full hover:bg-red-600 hover:text-white uppercase transition-colors"
                    >
